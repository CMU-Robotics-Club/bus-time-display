...
uint8_t get_row_bits(int col, int col_offset) {
  col += col_offset;
  col %= LETTER_WIDTH * message.length();
  
  int letter_col = col / LETTER_WIDTH;
  int inner_col = col % LETTER_WIDTH;

  return FONT[font_index(message.charAt(letter_col))][inner_col];
}
